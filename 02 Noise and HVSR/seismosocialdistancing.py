...
def stack_wday_time(df,scale):
    ...
    try:
        ...
        #ugly hack for now
        return df.groupby(level=(0, 1)).median().unstack(level=-1).T.droplevel(0)[["Sunday"]] * scale
...
def plot(displacement_RMS,
    ...
    for channelcode in list(set([k[:-1] for k in displacement_RMS])):
        ...
        if len(data.keys())>1:
            data[channelcode[-2:]+'*'] = data[main].copy().resample("".join(resample)).mean().shift(resample[0], resample[1]) # for the sum
            main=channelcode[-2:]+'*'
            ...
            for o in data:
                if o == main:
                    continue
                data[o] = data[o].copy().resample("".join(resample)).mean().shift(10, resample[0], resample[1])
                for i,t in enumerate(data[main].index):
        ...
        data[main] = localize_tz_and_reindex(data[main], "".join(resample), time_zone = time_zone)
        basename = "%s%s-%s"%(save,
                              channelcode[:]+main[-1],
                              band)
        ...
        if type in ['*', 'all', 'clockplots', 'dailyplots']:
            ...
            postloc = data[main].loc["2024-04-14 00:00:00":"2024-04-15 00:00:00"]
